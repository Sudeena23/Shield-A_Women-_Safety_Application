    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/95 backdrop-blur-xl p-4 animate-in fade-in duration-200">
      <div className="relative w-full max-w-sm bg-[#1a0c05] border border-[#4a2b18] rounded-3xl shadow-2xl overflow-hidden text-white flex flex-col justify-between min-h-[520px]">
        ...
        <div className="p-4 flex items-center justify-between border-b border-[#381b0a] bg-[#221007]">
          <div className="flex items-center gap-1.5 text-[11px] font-bold text-[#cb9d75]">
            <Radio className="w-3.5 h-3.5 text-emerald-400 animate-pulse" />
            <span>Shield Safety Escape Call</span>
          </div>
          <button
        ...
        {stage === 'ringing' && (
          <div className="p-6 flex-1 flex flex-col justify-between items-center text-center bg-gradient-to-b from-[#1a0c05] via-[#28150a] to-[#1a0c05]">
            ...
            <div className="w-full my-4">
              <div className="text-[10px] uppercase font-bold text-[#cb9d75]/70 mb-2 flex items-center justify-center gap-1">
                <Sparkles className="w-3 h-3 text-amber-400" /> Choose Caller Persona:
              </div>
              <div className="grid grid-cols-2 gap-1.5">
                {personas.map((p, idx) => (
                  <button
                    ...
                    className={`text-[11px] font-bold p-1.5 rounded-xl border transition-all text-left truncate cursor-pointer ${
                      selectedPersonaIndex === idx
                        ? 'bg-[#9e6133] border-amber-400 text-white shadow-md'
                        : 'bg-[#28150a] border-[#4a2b18] text-[#cb9d75] hover:text-white'
                    }`}
                  >
                    <div className="truncate font-black">{p.name}</div>
                    <div className="text-[9px] opacity-80 truncate">{p.tag}</div>
                  </button>
                ))}
              </div>
            </div>
        ...
        {stage === 'connected' && (
          <div className="p-5 flex-1 flex flex-col justify-between items-center text-center bg-gradient-to-b from-[#1a0c05] via-[#28150a] to-[#1a0c05]">
            ...
            {/* Conversation Script Prompt Box (Pretend Conversation Guide) */}
            <div className="w-full my-3 bg-[#221007] border border-[#522f18] rounded-2xl p-3 text-left space-y-2 shadow-inner">
              <div className="flex items-center justify-between text-[10px] font-extrabold uppercase text-[#cb9d75] tracking-wider">
                <span className="flex items-center gap-1">
                  <MessageSquare className="w-3 h-3 text-amber-400" />
                  Pretend Dialogue Script (Read out loud):
                </span>
              </div>

              <p className="text-xs italic text-[#eee0ce] bg-[#1a0c05] p-3 rounded-xl border border-[#3d2212] min-h-[52px] flex items-center transition-all">
                "{currentPersona.phrases[activePhraseIndex]}"
              </p>

              <div className="flex items-center justify-between text-[10px] text-[#cb9d75]">
                <span>Tap below to cycle conversation prompts:</span>
                <span className="font-mono">{activePhraseIndex + 1}/{currentPersona.phrases.length}</span>
              </div>
            </div>

            {/* Quick Trigger Buttons to change current prompt */}
            <div className="w-full space-y-1 text-left my-1">
              <div className="grid grid-cols-2 gap-1.5">
                {currentPersona.phrases.slice(0, 4).map((phrase, idx) => (
                  <button
                    key={idx}
                    onClick={() => setActivePhraseIndex(idx)}
                    className={`text-[10px] font-semibold p-2 rounded-xl border text-left truncate transition-colors cursor-pointer ${
                      activePhraseIndex === idx
                        ? 'bg-[#9e6133] border-amber-400 text-white font-bold'
                        : 'bg-[#2e180d] hover:bg-[#4a2b18] text-[#eee0ce] border-[#522f18]'
                    }`}
                    title={phrase}
                  >
                    💬 "{phrase.substring(0, 24)}..."
                  </button>
                ))}
              </div>
            </div>

            {/* In-Call Action Controls (Mute, Speaker) */}
            <div className="w-full grid grid-cols-2 gap-2 my-2">
              <button
                ...
                className={`p-2.5 rounded-xl border text-xs font-bold flex items-center justify-center gap-2 cursor-pointer transition-colors ${
                  isMuted ? 'bg-red-950/80 border-red-800 text-red-300' : 'bg-[#28150a] border-[#4a2b18] text-[#cb9d75] hover:text-white'
                }`}
              >
              ...
              <button
                ...
                className={`p-2.5 rounded-xl border text-xs font-bold flex items-center justify-center gap-2 cursor-pointer transition-colors ${
                  isSpeakerOn ? 'bg-amber-950/80 border-amber-800 text-amber-300' : 'bg-[#28150a] border-[#4a2b18] text-[#cb9d75]'
                }`}
              >
                {isSpeakerOn ? <Volume2 className="w-4 h-4 text-amber-400" /> : <VolumeX className="w-4 h-4" />}
